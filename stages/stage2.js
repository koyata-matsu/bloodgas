 ...
 * 0: 代謝性アシドーシス　合併なし
 * 1: 代謝性アルカローシス　合併なし
 * 2: 代謝性アシドーシス 呼吸性アルカローシス合併
 * 3: 代謝性アルカローシス　呼吸性アシドーシスの合併あり
 */
...
function makeBank() {
  ...
  for (let i = 0; i < 60; i++) {
    ...
    push(ph, paco2, hco3, 0);
  }
  // 呼吸性アルカローシス合併：PaCO2 < pred
  for (let i = 0; i < 60; i++) {
    const hco3 = randInt(6, 18);
    const pred = predPaCO2(hco3);
    const paco2 = Math.max(10, pred - randInt(6, 14));
    const ph = Number((7.40 - (24 - hco3) * 0.015).toFixed(2));
    push(ph, paco2, hco3, 2);
  }
  ...
  return {
    ...
    lessonHTML: `
      <div class="lessonBox">
        ...
        <div class="oneBlock">
          <div>代謝性アシドーシスの時は、<b>呼吸性アルカローシス</b>があるか</div>
    ...
    startDesc: "HCO₃ + 15 で予測PaCO₂を出して、合併の有無を4択で判定。",
